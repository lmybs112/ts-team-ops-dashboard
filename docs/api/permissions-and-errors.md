...
| 欄位 | 內容 |
|------|------|
| 對齊 | PRD v1.3 §6、ADR-002（單一 Issue 資料源）、ADR-003（Ingest 冪等與權限） |
...
5. **Ingest 身分綁定**：職缺 bot 的 token role 必須等於 body.`assigneeRole`，否則 `403`。
...
| 403 | `FORBIDDEN_ASSIGNEE` | bot 改他人任務，或 ingest role 不符 | 否 |
...
| P10 | ingest `assigneeRole` ≠ token role | 403 |
| P11 | ingest 非法 `projectSlug` | 400 `INVALID_ENUM` |
| P12 | CTO `DELETE` Issue | 204 |
| P13 | bot `DELETE` | 403 |

實作前由單元／TDD 職缺把 P1–P13 寫成失敗測試；後端實作僅在紅燈存在後進行。
